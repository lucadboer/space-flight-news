  ...
  return (
    ...
    <main className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <a href={post.url} target={'_blank'} className="bg-orange py-1 px-2 rounded cursor-pointer transition hover:saturate-[1.4]" rel="noreferrer">Ir para notícia</a>
      </div>
      <p className="text-gray leading-relaxed">{post.summary}</p>
      
      <Dialog.Root>
          <Dialog.Trigger asChild>
            <button className="mr-auto bg-orange py-1 px-3 rounded transition hover:saturate-[1.4]">Ver mais</button>
          </Dialog.Trigger>
          <NewsModal post={post} />
      </Dialog.Root>
    </main>
